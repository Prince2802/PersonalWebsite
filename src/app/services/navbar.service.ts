...
export class NavbarService {
  ...
  private setMenuBurger(): void {
    ...
    if (this.burger.classList.contains('is-active'))
      this.changeDivsDisplay(
        [this.body, this.footer, this.menu],
        ['none', 'none', 'block']
      );
    else
      this.changeDivsDisplay(
  ...
  public setBurgerMenuSettings(isActive: boolean): void {
    const toggle: NodeListOf<HTMLInputElement> =
      document.querySelectorAll('.toggle-checkbox');
  ...
  private setProperties(): void {
    this.menu = this.menu
      ? this.menu
      : (document.querySelector('.burger-menu') as HTMLDivElement);
    this.body = this.body
      ? this.body
      : (document.querySelector('.maindiv') as HTMLDivElement);
    this.burger = this.burger
      ? this.burger
      : (document.querySelector('.hamburger') as HTMLDivElement);
    this.footer = this.footer
      ? this.footer
      : (document.querySelector('footer') as HTMLDivElement);
    this.themeButton = this.themeButton
      ? this.themeButton
      : (document.querySelector('.toggle-checkbox') as HTMLInputElement);
  }
